...
public class Inferencer {	
	...
	public Model inference( LDADataset newData){
		logger.log(Level.FINE,"init new model");
		Model newModel = new Model();		
		
		newModel.initNewModel(option, newData, trnModel);		
		this.newModel = newModel;		
		
		//initialiaze for repeated mode (RIEDL)
		values = new ArrayList<int[][]>();
		for (int doc = 0; doc < newModel.z.length; doc++) {
			values.add(new int[newModel.z[doc].size()][newModel.K]);
		}
		
		//-----------------------
		logger.log(Level.FINE,"Sampling " + niters + " iteration for inference!");		
//		TopicTiling.printDim(newModel.z);
		...
		for (newModel.liter = 1; newModel.liter <= niters; newModel.liter++){
			...
			// for all newz_i
			for (int m = 0; m < newModel.M; ++m){//num of docs
		...
		logger.log(Level.FINE,"Gibbs sampling for inference completed!");
...
//		Model newModel = new Model();
	...
	public Model inference(List<String> [] strs){
		//System.out.println("inference");
//		Model newModel = new Model();
	...
	public Model inference(){	
		...
		logger.log(Level.INFO,"Sampling " + niters + " iteration for inference!");
		...
		logger.log(Level.FINE,"Gibbs sampling for inference completed!");		
		logger.log(Level.FINE,"Saving the inference outputs!");
